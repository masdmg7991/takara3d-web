...
$OrderAttributionFlowValidatorPath = Join-Path $Project "tools/takara_validar_order_attribution_flow.py"

if (Test-Path $OrderAttributionFlowValidatorPath) {
    Log-Line ""
    Log-Line "[RUN] py tools/takara_validar_order_attribution_flow.py"
    py tools/takara_validar_order_attribution_flow.py 2>&1 | ForEach-Object { Log-Line $_ }

    if ($LASTEXITCODE -eq 0) {
        Ok "F3D real doPost attribution wiring static validado"
    } else {
        Err "Fallo takara_validar_order_attribution_flow.py"
    }
} else {
...
if (Test-Path $StoreAdminSystemF4FValidatorPath) {
    Log-Line ""
    Log-Line "[RUN] py tools/takara_validar_store_admin_system_f4f.py"
    py tools/takara_validar_store_admin_system_f4f.py 2>&1 | ForEach-Object { Log-Line $_ }
    if ($LASTEXITCODE -eq 0) { Ok "F4F Store Admin deployment/SystemScenario static validado" } else { Err "Fallo takara_validar_store_admin_system_f4f.py" }
} else {
    Err "No existe tools/takara_validar_store_admin_system_f4f.py"
}
...
if (Test-Path $StoreAdminWriteValidatorPath) {
    Log-Line ""
    Log-Line "[RUN] py tools/takara_validar_store_admin_write.py"
    py tools/takara_validar_store_admin_write.py 2>&1 | ForEach-Object { Log-Line $_ }
    if ($LASTEXITCODE -eq 0) { Ok "F4D Store Admin write static validado" } else { Err "Fallo takara_validar_store_admin_write.py" }
} else {
    Err "No existe tools/takara_validar_store_admin_write.py"
}
...
if (Test-Path $StoreAdminAccessValidatorPath) {
    Log-Line ""
    Log-Line "[RUN] py tools/takara_validar_store_admin_access.py"
    py tools/takara_validar_store_admin_access.py 2>&1 | ForEach-Object { Log-Line $_ }
    if ($LASTEXITCODE -eq 0) { Ok "F4A Store Admin access static validado" } else { Err "Fallo takara_validar_store_admin_access.py" }
} else {
    Err "No existe tools/takara_validar_store_admin_access.py"
}
...
if ($null -ne $NodeCommand -and (Test-Path "tools/takara_test_store_admin_system_f4f.js")) {
    ...
    node tools/takara_test_store_admin_system_f4f.js 2>&1 | ForEach-Object { Log-Line $_ }
    if ($LASTEXITCODE -eq 0) { Ok "F4F Store Admin deployment/SystemScenario functional validado" } else { Err "Fallo takara_test_store_admin_system_f4f.js" }
} else {
    Err "No se pudo ejecutar F4F Store Admin SystemScenario"
}
...
if ($null -ne $NodeCommand -and (Test-Path "tools/takara_test_store_admin_write.js")) {
    ...
    node tools/takara_test_store_admin_write.js 2>&1 | ForEach-Object { Log-Line $_ }
    if ($LASTEXITCODE -eq 0) { Ok "F4D Store Admin write functional validado" } else { Err "Fallo takara_test_store_admin_write.js" }
} else {
    Err "No se pudo ejecutar F4D Store Admin write test"
}
...
if ($null -ne $NodeCommand -and (Test-Path "tools/takara_test_store_admin_access.js")) {
    ...
    node tools/takara_test_store_admin_access.js 2>&1 | ForEach-Object { Log-Line $_ }
    if ($LASTEXITCODE -eq 0) { Ok "F4A Store Admin access functional validado" } else { Err "Fallo takara_test_store_admin_access.js" }
} else {
    Err "No se pudo ejecutar F4A Store Admin access test"
}
...
if ($null -ne $NodeCommand -and (Test-Path "tools/takara_test_order_attribution_flow.js")) {
    ...
    node tools/takara_test_order_attribution_flow.js 2>&1 | ForEach-Object { Log-Line $_ }

    if ($LASTEXITCODE -eq 0) {
        Ok "F3D real doPost attribution wiring functional validado"
    } else {
        Err "Fallo takara_test_order_attribution_flow.js"
    }
} else {
    Err "No se pudo ejecutar F3D doPost attribution flow test"
}
