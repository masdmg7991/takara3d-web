...
def main() -> int:
    ...
    require(
        ...
        f"Code.gs SHA protegido exacto; actual={code_sha}",
    )
    ...
    require(
        not re.search(r"(?m)^[ \t]*function[ \t]+doGet[ \t]*\(", bridge),
        "StoreAdminUiBridge no crea un segundo doGet",
    )
    require(
        not re.search(r"(?m)^[ \t]*function[ \t]+doPost[ \t]*\(", bridge),
        "StoreAdminUiBridge no crea un segundo doPost",
    )

    require(
        "TAKARA_GET_APPS_SCRIPT_ENDPOINT" in config,
        "Endpoint Apps Script sigue centralizado en takara-config.js",
    )
    require(
        "TAKARA_GET_APPS_SCRIPT_ENDPOINT" in store_public_js,
        "Store Public JS consume la autoridad central del endpoint",
    )
    ...
    require(
        config_ref >= 0,
        "Store Public HTML carga takara-config.js",
    )
    require(
        store_js_ref >= 0,
        "Store Public HTML carga takara-store-public.js",
    )
    require(
        config_ref < store_js_ref,
        "Store Public carga config antes de su consumidor",
    )

    require(
        "getStoreAdminUiDeploymentOutput_" not in do_get,
        "Admin boundary todavía no está integrado en doGet",
    )

    get_sha = hashlib.sha256(
        do_get.encode("utf-8")
    ).hexdigest().upper()
