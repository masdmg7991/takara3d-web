...
EXPECTED = {
    ...
    "script": "TAKARA_PEDIDOS_WEB_APPS_SCRIPT_V1_15_0_ORDER_IDEMPOTENCY_V1",
}
...
def main() -> int:
    ...
    require(EXPECTED["script"] in code, "Apps Script usa versión V1.15.0 order idempotency")
    ...
    require(
        ...
        "Validador de personalización exige Apps Script V1.15.0 order idempotency",
    )
    ...
    for doc, name in ((readme, "README"), (deployment, "DEPLOYMENT"), (contract, "ORDER_ENGINE_CONTRACT")):
        require(EXPECTED["script"] in doc, f"{name} documenta Apps Script V1.15.0 order idempotency")
